...
app.post('/borrow-book', (req, res) => {
    if (!req.session.user || req.session.user.role !== 'member') return res.status(403).send('❌ Only members can borrow books');
...
// History pages
app.get('/history', (req, res) => { if (!req.session.user) return res.redirect('/'); res.sendFile(path.join(__dirname, 'views', 'history.html')); });
...
app.post('/return-book', (req, res) => {
    if (!req.session.user || req.session.user.role !== 'member') return res.status(403).send('❌ Only members can return books');
